...
exports.createFactory = async (req, res) => {
  ...
  try {
    ...
    if (!party) {
      ...
      session.endSession();
      // --- Logging ---
      logger.warn(`Attempted to create factory for non-existent Party ID: ${party_id}`);
      return res.status(404).json({ message: 'Party not found. Cannot create factory.' });
    }
    ...
    // --- Logging ---
    logger.info(`Factory created: '${name}' (ID: ${newFactory._id}) and linked to Party '${party.name}' (ID: ${party_id})`);
    ...
    // --- Logging ---
    if (error.code === 11000) {
        logger.warn(`Attempted to create duplicate Factory: '${name}' for Party ID: ${party_id}`);
        return res.status(409).json({ error: 'This factory name already exists for the specified party.' });
    }
    logger.error(`Failed to create factory '${name}': ${error.message}`, { requestBody: req.body });
...
exports.getAllFactories = async (req, res) => {
  try {
    ...
    // --- Logging ---
    logger.info('Successfully retrieved all factories.');
    
    res.status(200).json(factories);
  } catch (error) {
    // --- Logging ---
    logger.error(`Failed to fetch all factories: ${error.message}`);
...
exports.getFactoryById = async (req, res) => {
    try {
        ...
        if (!factory) {
            // --- Logging ---
            logger.warn(`Factory not found for ID: ${req.params.id}`);
            return res.status(404).json({ message: 'Factory not found.' });
        }
        
        // --- Logging ---
        logger.info(`Successfully retrieved factory: ${factory.name} (ID: ${req.params.id})`);
        
        res.status(200).json(factory);
    } catch (error) {
        // --- Logging ---
        logger.error(`Failed to fetch factory with ID ${req.params.id}: ${error.message}`);
...
exports.updateFactory = async (req, res) => {
  ...
  try {
    ...
    if (!updatedFactory) {
      // --- Logging ---
      logger.warn(`Attempted to update non-existent Factory with ID: ${id}`);
      return res.status(404).json({ error: 'Factory not found.' });
    }

    // --- Logging ---
    logger.info(`Factory updated: ${updatedFactory.name} (ID: ${id})`);

    res.status(200).json({ message: 'Factory updated successfully', data: updatedFactory });
  } catch (error) {
    // --- Logging ---
    logger.error(`Failed to update factory with ID ${id}: ${error.message}`, { requestBody: req.body });
...
exports.deleteFactory = async (req, res) => {
  ...
  try {
    ...
    if (!factory) {
      ...
      session.endSession();
      // --- Logging ---
      logger.warn(`Attempted to delete non-existent Factory with ID: ${id}`);
      return res.status(404).json({ error: 'Factory not found.' });
    }
    ...
    // --- Logging ---
    logger.info(`Factory deleted: ${factory.name} (ID: ${id})`);
    ...
    // --- Logging ---
    logger.error(`Failed to delete factory with ID ${id}: ${error.message}`);
