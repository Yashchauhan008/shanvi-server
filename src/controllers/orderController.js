...
exports.addOrder = async (req, res) => {
  ...
  try {
    ...
    // --- Logging ---
    logger.info(`Transaction created: ${order.customOrderId}`);
    ...
    // --- Logging ---
    logger.error(`Failed to add order/bill: ${error.message}`, { requestBody: req.body });
...
exports.getOrders = async (req, res) => {
  try {
      ...
      // --- Logging ---
      logger.info(`Retrieved ${orders.length} of ${total} orders for page ${page}`);
      ...
      // --- Logging ---
      logger.error(`Failed to retrieve orders: ${error.message}`, { query: req.query });
...
exports.getPalletStats = async (req, res) => {
  try {
    ...
    const palletStats = await Order.aggregate([
      { $match: matchStage },
      { $unwind: '$items' },
      { $group: { _id: '$items.paletSize', totalOut: { $sum: { $cond: [{ $eq: ['$transactionType', 'order'] }, '$items.quantity', 0] } }, totalIn: { $sum: { $cond: [{ $eq: ['$transactionType', 'bill'] }, '$items.quantity', 0] } } } },
      { $project: { _id: 0, palletSize: '$_id', totalOut: '$totalOut', totalIn: '$totalIn', netBalance: { $subtract: ['$totalOut', '$totalIn'] } } },
      { $sort: { palletSize: 1 } }
    ]);

    // --- Logging ---
    logger.info('Successfully retrieved pallet statistics.');
    ...
    // --- Logging ---
    logger.error(`Failed to retrieve pallet statistics: ${error.message}`, { query: req.query });
...
exports.getOrderById = async (req, res) => {
  try {
    ...
    // --- Logging ---
    logger.info(`Successfully retrieved order: ${order.customOrderId}`);

    res.status(200).json(order);

  } catch (error) {
    // --- Logging ---
    logger.error(`Failed to fetch order with ID ${req.params.id}: ${error.message}`);
...
exports.deleteOrder = async (req, res) => {
  ...
  try {
    ...
    // --- Logging ---
    logger.info(`Transaction deleted: ${order.customOrderId} (ID: ${id})`);
    ...
    // --- Logging ---
    logger.error(`Failed to delete order with ID ${id}: ${error.message}`);
...
exports.findOrderByCustomId = async (req, res) => {
  try {
    ...
    if (!order) {
      // --- Logging ---
      logger.warn(`Search by custom ID failed. No record found for: '${customId}'`);
      return res.status(404).json({ message: `No record found with the ID '${customId}'.` });
    }

    // --- Logging ---
    logger.info(`Search by custom ID successful. Found: ${order.customOrderId}`);
    ...
    // --- Logging ---
    logger.error(`Server error during search for custom ID '${req.params.customId}': ${error.message}`);
...
exports.updateOrder = async (req, res) => {
  ...
  try {
    ...
    // --- Logging ---
    logger.info(`Transaction updated: ${updatedOrder.customOrderId} (ID: ${id})`);
    ...
    // --- Logging ---
    logger.error(`Failed to update transaction with ID ${id}: ${error.message}`, { requestBody: req.body });
